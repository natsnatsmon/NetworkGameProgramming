...
DWORD WINAPI ClientMain(LPVOID arg) {
	...
	while (1) {
		...
		//	break;
		//}
		//DisplayText((char*)"[TCP 클라이언트] %d바이트를 보냈습니다.\r\n", retVal);
		...
		DisplayText((char*)"[TCP 클라이언트] 파일 이름 (%d +) %d 바이트를 보냈습니다. \r\n", sizeof(int), retVal);
		...
		DisplayText((char*)"[TCP 클라이언트] 파일 크기 %dByte를 담은 %d바이트를 보냈습니다.\r\n", fileSize, sizeof(int));
		...
		DisplayText((char*)"[TCP 클라이언트] 파일 업로드 완료! 보낸 파일 크기 : %d Byte\r\n", uploadFileSize);
